topSuite("Ext.grid.plugin.Exporter", ['Ext.exporter.*', 'Ext.grid.*', 'Ext.data.*'], function() {
    ...
    describe('exporters', function() {
        ...
        it('should generate an xlsx cell correctly', function () {
            ...
            ws.setName('Demo*ORBIT*Nagaraju*Hyderabad*Sales1');
            expect(ws.getName()).toBe('DemoORBITNagarajuHyderabadSales');
            ws.setName('Demo/ORBIT*Nagaraju\Hyderabad?Sales:[]123');
            expect(ws.getName()).toBe('DemoORBITNagarajuHyderabadSales');
            ws.setName('Demo/ORBIT*Nagaraju\Hyderabad?Sales:[]1234');
            expect(ws.getName()).toBe('DemoORBITNagarajuHyderabadSa...');
            ws.setName(null);
            expect(ws.getName()).toBe('Sheet1');

            // check if name is unique
            ws.setName('test');
            expect(ws.getName()).toBe('test');
            ws2.setName('test');
            expect(ws2.getName()).toBe('Sheet2');
        ...
        it('should generate an excel03 cell correctly', function () {
            var excel = new Ext.exporter.file.excel.Workbook(),
                d = new Date(),
                checkFn = function(value, compare) {
                    var cell, v;
                    ...
                    v = String(cell.render()).replace(/ *</g, '<');
                    v = v.replace(/\n/g, '');
                    expect(v).toBe(compare);
                },
                ws, table, row;
            ...
            checkFn('test', '<Cell><Data ss:Type="String">test</Data></Cell>');
            checkFn(null, '<Cell><Data ss:Type="String"></Data></Cell>');
            checkFn(undefined, '<Cell><Data ss:Type="String"></Data></Cell>');
            checkFn(0, '<Cell><Data ss:Type="Number">0</Data></Cell>');
            checkFn(5.3, '<Cell><Data ss:Type="Number">5.3</Data></Cell>');
            checkFn(d, '<Cell><Data ss:Type="DateTime">' + Ext.Date.format(d, 'Y-m-d\\TH:i:s.u') + '</Data></Cell>');
            checkFn(true, '<Cell><Data ss:Type="Boolean">1</Data></Cell>');
            checkFn(false, '<Cell><Data ss:Type="Boolean">0</Data></Cell>');
        });
    });
